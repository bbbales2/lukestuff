...
def buildFeatures(data, b = 8):
    ...
    for index, row in data.iterrows():
        im = skimage.io.imread(row['f'], as_grey = True).astype('double')
        im -= numpy.mean(im)
        im /= numpy.std(im)
...
toplot = svm.predict(hist.reshape((-1, hist.shape[-1]))).reshape((hist.shape[0], hist.shape[1]))
...
for feature in ["pct", "core", "aged", "temp", "type"]:
    ...
    for index, row in test.iterrows():
        ...
        features = skimage.feature.local_binary_pattern(im, 7, 3.0)
        hist = microstructure.features.labels2boxes(features, int(2**7), size = b + 1, stride = b, padding_mode = 'reflect')
        boxes = microstructure.features.hists2boxes(hist, b = 9, padding_mode = 'reflect')
